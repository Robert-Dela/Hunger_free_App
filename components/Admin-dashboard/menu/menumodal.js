...
export default function MealModal({ showmodal, setshowmodal }){
  ...
  const add_menu_url = `${API_BASE_URL}:${API_PORT}/${Menu_endpoint}`;
  ...
  //function for adding new meal to menu 
  const onSubmit = (data = {day, mealname}) => {
    

    console.log('data is ', data)
    axios.post( add_menu_url, 
...
.then(resp =>{
      ...
      // On successful addition of meal to menu 
      console.log(resp)
      
      if (resp.status == 201 || 200) {

        //store added menu details

        localStorage.setItem('menu', resp.data)
                            ...
                            <div className={form}>
                                <Form onSubmit={handleSubmit(onSubmit)}>
                                    ...
                                    <div className={form_group}>
                                        <label for="day" className={form_label}>Day</label>                                    
                                        <select id="day" name="day" className={form_input} 
